...
export function StatusForm({ table, row }: {
    ...
    const states = table === "reviews" ? ["pendiente", "aprobada", "rechazada", "despublicada"] : ["nuevo", "contactado", "cerrado"];
    return <form action={action} className="mt-4 flex flex-wrap items-end gap-4"><input type="hidden" name="table" value={table}/><input type="hidden" name="id" value={String(row.id)}/><label className="t-small">Estado<select className={control} name="estado" defaultValue={String(row.estado)}>{states.map(s => <option key={s} value={s}>{s === "despublicada" ? "Despublicada" : s}</option>)}</select></label>{table === "reviews" && <label className="flex items-center gap-2"><input type="checkbox" name="verificada" defaultChecked={row.verificada === true}/>Verificada</label>}<Submit pending={pending}/>{table === "reviews" && <><Button type="submit" variant="ghost" disabled={pending} name="accion_estado" value="aprobada">Aprobar</Button><Button type="submit" variant="ghost" disabled={pending} name="accion_estado" value="rechazada">Rechazar</Button><Button type="submit" variant="ghost" disabled={pending} name="accion_estado" value="despublicada">Despublicar</Button></>}<Message state={state}/></form>;
}

    ...
    <QuoteButton
      payload={{
        ...
        fields: {
          ...
          Paquete: item.nombre,
          Notas: "Por favor, comparta sus fechas y el número de pasajeros.",
        },
        formData: { slug: item.slug, nombre: item.nombre, destino: item.destino },
      }}
    >
  ...
  if (!items.length) {
    return (
      <div role="status" className="rounded-panel border border-line bg-surface p-8 text-center">
        <p className="t-body text-ink-soft">Pronto publicaremos nuevos paquetes.</p>
      </div>
    );
  }

  return <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{items.map(item => <PackageCard key={item.id} item={item} />)}</div>;
}

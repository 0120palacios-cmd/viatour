export function PackageSkeletons() {
  return (
    <div role="status" aria-label="Cargando paquetes">
        ...
        {[0, 1, 2].map(key => (
          <div key={key} className="overflow-hidden rounded-card border border-line bg-canvas p-6 shadow-sm motion-safe:animate-pulse">
            ...
            <div className="space-y-4 pt-6">
              <div className="h-8 w-3/4 rounded-btn bg-surface" />
              <div className="h-5 rounded-btn bg-surface" />
              <div className="h-12 rounded-btn bg-surface" />
            </div>
          </div>
        ))}
      </div>

...
export function RequirementsChecker() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setBusy(true); setResult(null); setRequestError(false);
    try {
      ...
      if (!response.ok || payload.status !== "ok") setRequestError(true);
  ...
  return <div className="space-y-8">
    ...
    {requestError && <p role="alert" className="rounded-card border border-line bg-surface p-5 t-body text-ink-soft">{t("lookupError")}</p>}
    {data && <section className="rounded-panel border border-line bg-canvas p-6 shadow-sm sm:p-8" aria-labelledby="requirements-result-title" aria-live="polite">
      ...
      <dl className="mt-8 grid gap-6 border-y border-line py-6 md:grid-cols-2"><div><dt className="t-small text-ink-soft">{t("visa")}</dt><dd className="t-body mt-2">{data.visaRequirement}</dd></div><div><dt className="t-small text-ink-soft">{t("stay")}</dt><dd className="t-body mt-2">{data.allowedStay}</dd></div><div className="md:col-span-2"><dt className="t-small text-ink-soft">{t("documents")}</dt><dd className="t-body mt-2"><ul className="list-disc space-y-2 pl-5">{data.passportRules.map(rule => <li key={rule}>{rule}</li>)}</ul></dd></div>{data.transitRequirements && <div className="md:col-span-2"><dt className="t-small text-ink-soft">{t("transit")}</dt><dd className="t-body mt-2">{data.transitRequirements}</dd></div>}</dl>

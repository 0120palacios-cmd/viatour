...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> { const [destinations, packages, posts] = await Promise.all([getDestinations(), getPackages(), getBlogPosts()]); const publishedCityPaths = CITY_SEO_PAGES.filter(city => city.published).map(city => `/agencia-de-viajes/${city.slug}`); return [...[...new Set(staticPaths)].flatMap(path => localizedEntries(path)), ...destinations.flatMap(item => localizedEntries(`/destinos/${item.slug}`)), ...packages.flatMap(item => localizedEntries(`/paquetes/${item.slug}`, item.updated_at || undefined)), ...posts.flatMap(item => localizedEntries(`/blog/${item.slug}`, item.updated_at || item.publicado_en || undefined)), ...publishedCityPaths.flatMap(path => localizedEntries(path))]; }

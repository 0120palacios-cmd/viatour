...
export default async function Page() {
    ...
    return <><div className="mb-6 flex flex-wrap items-center justify-between gap-4"><h1 className="t-h1">Resumen</h1><Link href="/admin/promociones" className="min-h-12 rounded-btn bg-brand px-6 py-3 text-canvas">Códigos de la ruleta</Link></div><div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{specs.map((spec, i) => <div key={spec[0]} className="rounded-card border bg-surface p-6"><h2 className="t-body text-ink-soft">{spec[0]}</h2><p className="t-h2 mt-2">{counts[i]}</p></div>)}</div></>;
}
